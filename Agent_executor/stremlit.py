...
async def run_analyser_gpt(docker,get_model_client,task):
    try:
        await start_docker_executor(docker)
        team = get_Data_Analyzer_Team(docker,get_model_client)
...
if task:
   if uploaded_file is not None: 
        ...
        get_model_client= get_model_client()
        docker = get_docker_commandline_executor()

        error = asyncio.run(run_analyser_gpt(docker,get_model_client,task))
